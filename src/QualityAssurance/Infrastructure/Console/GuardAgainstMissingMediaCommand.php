...
class GuardAgainstMissingMediaCommand extends Command {
    ...
    public const TWEET_MATCHING_PATTERN =
        ...
        '(:?[^"]+)|NULL)"?,(?<isPublished>.+)' .
        '#sm';
    public const OPTION_SKIP_TWEET_MEDIA = 'skip-tweet-media';
    ...
    public function configure()
    {
        $this->setName(self::COMMAND_NAME)
            ...
            ->addOption(
                self::OPTION_SKIP_TWEET_MEDIA,
                mode: InputOption::VALUE_NONE,
                description: 'Skipping retrieval of tweets cover media'
            )
        ;
    }
    ...
    public function makeTweets(string $filePath): array
    {
        ...
        preg_match_all('#(^\d+,".*,(?:False|True))\s#m', $contents, $lines);

        return array_reduce($lines[1], function ($carry, $item) {
            preg_match(
                self::TWEET_MATCHING_PATTERN,
                $item,
                $matches
            );
