	...
	)
	{
		...
		)
		{
			...
			$operand_pattern =
				REGEXP_OPEN.
					$class_application::translate_entity(
						PROPERTY_LEFT_OPERAND, ENTITY_PATTERN
					).
					SYMBOL_TRIPLE_EQUAL.
					REGEXP_CATCH_START.
					...
					REGEXP_OR.
					$class_application::translate_entity(
						PROPERTY_RIGHT_OPERAND, ENTITY_PATTERN
					).
					SYMBOL_TRIPLE_EQUAL.
					REGEXP_CATCH_START.
						REGEXP_WILDCARD.REGEXP_FACULTATIVE.
			...
			while (list($operand_index, $operand) = each($operands))
			{
				...
				$operand_match = preg_match(
					$operand_pattern, $operand_index, $operand_matches
				);
		...
		$class_dumper::log(
			__METHOD__,
			array( 'returned value', $processed_operands ),
			FALSE
		);
