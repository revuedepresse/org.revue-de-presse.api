...
class Api extends Transfer
{
	...
	* @return  boolean	TRUE on close success
	*/
	public static function closeImapStream( $stream = NULL )
	{
	...
	*/
	public static function dumpImapLabels( $mailbox = NULL, $resource = NULL )
	{
		...
		$file_path =
			dirname( __FILE__ ).'/../../'.
			DIR_API . DIR_IMAP . CHARACTER_SLASH . $file_name
		;
		$list = array();
		$mailboxes = '';
		...
		if ( is_null( $resource ) ) $resource = self::openImapStream( $mailbox );
		...
		$closing_result = self::closeImapStream( $resource );

		$class_dumper::log( __METHOD__, array(
				'[closing result]' , $closing_result ? 'TRUE' : 'FALSE'
		) );
		...
		file_put_contents( $file_path, $mailboxes );

		$class_dumper::log( __METHOD__, array(
				'[imap labels saved to file "' . $file_path . '"]'
		), $verbose_mode );
	...
	*/
	public static function openImapStream( $stream = NULL )
	{
		...
		if ( is_null( $stream ) ) $stream = self::getImapStream();
		...
		try {
			...
			if (
				$_stream = imap_open(
					$stream, IMAP_USERNAME, IMAP_PASSWORD
				)
			)
				$stream = $_stream;
