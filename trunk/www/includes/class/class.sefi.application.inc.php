...
/**
* sefi namespace created under Weaving The Web project
*
* @package  sefi
*/
...
* Application class
*
* Class for bootstrapping an application
* @package  sefi
*/
class Application extends \Service_Manager
{
	...
	*/
	public static function destroySession( $administrator = FALSE )
	{
		...
		{
			if ( ! $use_symfony ) session_start(); // start a session
	...
	*/	
	...
	)
	{
		...
		// fetch a media
		$file_content = self::fetchMedia(
			$media, $media_type, $properties
		);

		$path_configuration_file_magic_mgc =
			PATH_CONFIG . '/' . CONFIGURATION_FILE_MAGIC_MGC
		;

		$file_mime_type = self::getResourceMimeType( $file_content );

		// check the key
		if (
			( $properties->{PROPERTY_KEY} !== TRUE )
			&& ( $file_mime_type != MIME_TYPE_PLAIN_TEXT )
		)			
	...
	*/	
	...
	)
	{
		...
		else
		{
			...
			$file_mime_type = self::getResourceMimeType( $resource );
	...
	)
	{
		...
		if ( empty( $identifier ) &&
			...
			$class_dumper::log( __METHOD__, array(
				'please provide a valid identifier (' . GET_IDENTIFIER . ')'
			) );
	...
	*/	
	...
	/**
	* Get a resource mime type
	*
	* @param	$resource
	* @return	string 	mime type
	*/
	public static function getResourceMimeType( $resource )
	{
		// construct a new instance of file information
		$finfo = new \finfo( FILEINFO_MIME );
		
		return $finfo->buffer( $resource, FILEINFO_MIME_TYPE);
	}
...
{
	public static function assert( $assertion, $line )
	{
		$assertion_true = assert( $assertion );

		$result =
			( $assertion_true ? '\O/ test passed ' : 'o_0 test failed ' ) .
			( 'at line # ' . $line )
		;
	...
	{
		return substr(
			__CLASS__, 0, strlen( __CLASS__ ) - strlen( '_Test' )
		); 
	}

	public static function report( $log, $verbose_mode )
	{
		$_log = $log;
		...
		if ( $verbose_mode )
			...
			foreach( $_log as $item )

				echo str_repeat( "\t", 1 ), $item . "\n";

		echo "\n";
	}

	public static function getResourceMimeTypeTest()
	{
		global $verbose_mode;
		$class = self::className();
		$function = __FUNCTION__;
		$_function = substr(
			$function, 0, strlen( $function ) - strlen( 'Test' )
		);

		$assertion = FALSE;
		$script_mockup =
			__DIR__ . '/../mockup' .
			'/Sefi/Application/Method/method.get_resource_mime_type.php'
		;

		require( $script_mockup );

		if ( isset( $argument ) )

			$assertion =
				call_user_func_array( array( $class, $_function ), array(
					unserialize( base64_decode(  $argument ) )
				) ) === 'image/jpeg'
			;
		...
		$log = array(
			'[testing ' . $class . '::' . $_function . ']',
			'[ Provided mockup resource is of correct mime type (image/jpeg) ]',
				self::assert( $assertion, __LINE__ ),
		);

		self::report( $log, $verbose_mode );
	}

	public static function validateMethods()
	{
		try {
			self::getResourceMimeTypeTest();
		} catch ( Exception $exception )
		{
...
*
* Mutualize retrieval of resource mime type
* Start implementing unit testing 
*
* method affected ::
*
* 	SEFI\APPLICATION :: getResourceMimeType
* 
