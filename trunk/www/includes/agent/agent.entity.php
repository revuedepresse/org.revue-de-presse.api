...
class Entity implements Model_Entity
{
	...
	*/	
	public static function __callStatic( $name, $arguments )
	{
		...
		if (
			isset( $arguments[1] ) && isset( $arguments[2] ) &&
			( $arguments[2] == 1024 )
		)
			$class_dumper::log(
				__METHOD__,
		...
		)
		{
			...
			)
			{
				...
				if (
					in_array( $method_name, get_class_methods( $entity_type ) )
				)
			...
			$match = preg_match( $pattern_get_instance_type, $name, $matches )
		)
		{
			$method_name =
				ACTION_GET.
			...
			$signature =
				static::getSignature() === __CLASS__ && isset( $matches[1] )
				? $matches[1] : static::getSignature()
			;

			if ( in_array( $method_name, get_class_methods( __CLASS__ ) ) )

				$callback_parameters = call_user_func_array(
					array( $signature, $method_name ),
					$arguments
				);
		}
	...
	*/	
	...
	)
	{
		...
		else if ( is_array( $properties ) )
		{
			...
			throw new Exception(
				EXCEPTION_CONSISTENCY_DATA_ACCESS_QUERY_INVALID
			);
	...
	*/	
	public static function getDefaultType(
		$property = PROPERTY_VALUE, $entity_type = NULL
	)
	{
		$callback_parameters = NULL;
		...
		if (
			is_object( $default_type ) &&
			isset( $default_type->{PROPERTY_VALUE} )
		)
		{
			if ( ! empty( $property ) )
			{
				if ( isset( $default_type->$property ) )
					...
					$exception =
						EXCEPTION_INVALID_ARGUMENT . ': ' .
							EXCEPTION_INVALID_PROPERTY_NAME
					;
			}
			else
			...
			$exception = sprintf(
				constant(
					'EXCEPTION_CONSISTENCY_DATA_ACCESS_' .
					'ENTITY_TYPE_DEFAULT_VALUE_MISSING'
				), static::getSignature()
			);
	...
	*/
	public static function getInstanceTypeProperty(
...
* support :: arc ::
*
* Revise signature disambiguation
*
* method affected ::
*
* 	ENTITY :: __callStatic
