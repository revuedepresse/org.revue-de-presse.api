...
class Serializer extends Executor
{
	...
	* @return	mixed
	*/
	public static function flagAsShared( $properties = NULL )
	{
		...
		/**
		* Inject the following variables into the symbol table
		* 
		* @tparam	integer		$id 		id
		* @tparam	boolean		$status		flag status
		*/
		...
		*/
		...
		# insert new edge representing a entity type concept 

		$query_insert_edge_destination = '
		...
		{
			$insertion_results = $class_db::query(
				$query_insert_edge_destination
			);
			...
			* FIXME
			*
			* Update existing disabled concept edge of type entity type 
			* if required but only if flag toggled is perfectly well identified
			*
			*/
			...
			if ( ! $insertion_results )
				...
				throw new Exception(
					sprintf(
						EXCEPTION_EXISTING_ENTITY,
						' '.ENTITY_RECORD
					)
				);

			$edge_destination_id = $link->insert_id;
		}
		else

			$edge_destination_id =
				$results_edge_destination->fetch_object()->{PROPERTY_ID}
			;
	...
	*/
	public static function saveEvent($context, $storage_model = STORE_DATABASE)
	{
		...
		if (is_array($context) && count($context) > 0)
		{
			...
			$update_event_occurrence_model = '
				UPDATE '.TABLE_EVENT.' SET
					'.$column_prefix.PROPERTY_OCCURRENCE.' = ' .
						$column_prefix.PROPERTY_OCCURRENCE.' + 1,
			...
			else
			{
				...
				$update_instance_occurrence = str_replace(
					'{id}',
					$event_id,
					$update_event_occurrence_model
				);

				if ($class_db::query($update_instance_occurrence))
				...
				if ( empty( $field_attributes[ACTION_MOVE_TO] ) )
				{
					if ( $match_serialization && ! empty( $matches[3] ) )
					{
						if (
							...
							$class_dumper::log( __METHOD__, array( sprintf(
								EXCEPTION_MISSING_PROPERTY,
								$field_attributes[HTML_ATTRIBUTE_NAME]
							) ), TRUE, TRUE );	
	...
	*/
	...
	)
	{
		...
		$path_folder_rdf = PATH_WTW . '/rdf';

		try {
			if ( ! file_exists( $path_folder_rdf ) )
			
				mkdir( $path_folder_rdf );
			
			else if ( ! is_dir( $path_folder_rdf ) )
	
				throw new Exception( sprintf(
					EXCEPTION_INVALID_ENTITY, ENTITY_FOLDER . ' (' . DIR_RDF . ')'
				) );
		}
		catch ( Exception $e )
		{
            $class_dumper::log( __METHOD__,
                array( $e ),
            DEBUGGING_DISPLAY_EXCEPTION || CLI_MODE, AFFORDANCE_CATCH_EXCEPTION );			
		}

		$rdf_file_name =
			$path_folder_rdf . '/' . $hash . $rewritten_file_name. EXTENSION_RDF
		;
...
*
* Always assign values as strings
*
* method affected ::
*
* SERIALIZER:: saveInstance
*
