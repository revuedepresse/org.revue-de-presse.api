...
* @package  sefi
*/
class Data_Fetcher extends \sefi\Database
{
    /**
    * Load a photograph
    * 
			...
			switch ( $entity_type )
			{
				...
				default:
					...
					)
					{
						...
						if (
							isset( $context[SQL_ORDER_BY] ) && (
								is_array( $context ) &&
								(
									$clause_ordery_is_array =
										is_array( $context[SQL_ORDER_BY] )
								) || str_valid( $context[SQL_ORDER_BY] )
							)
						)
						{
							$clause_order_by_start = "\n".SQL_ORDER_BY."\n";

							if (
								$clause_ordery_is_array &&
								count( $context[SQL_ORDER_BY] ) > 0
							)
							{
								$selector = $table_alias.'.'.$column_prefix;
								
								foreach (
									$context[SQL_ORDER_BY]
										as $column_index => $column
								)
								{
									if (
										! is_array( $column ) &&
										...
										$_columns[$column_index] = array(
											PROPERTY_COLUMN => $selector.$column,
											PROPERTY_TYPE => ORDER_BY_TYPE_DESC
										);
									else if (
										! isset( $column[PROPERTY_TYPE] ) ||
										! isset( $column[PROPERTY_COLUMN] )
									)
										throw new Exception(
											EXCEPTION_SQL_INVALID_CLAUSE_ORDER_BY
										);
									else
										$_columns[$column_index] = array(
											PROPERTY_COLUMN =>
												$selector.$column[PROPERTY_COLUMN],
											PROPERTY_TYPE => $column[PROPERTY_TYPE]
										); 
	
									$order_by_column[$column_index] = implode(
										' ', $_columns[$column_index]
									);
	
									$clause_order_by = implode(
										', ', $order_by_column
									);
								}
								
								if ( str_valid( $clause_order_by ) )
	...
	*/	
	public static function __callStatic( $name, $arguments )
	{
		...
		$operation_get_entity_type = str_replace(
			'.', '', ACTION_GET_ENTITY_TYPE
		);
		...
		if (
			FALSE !==
			strpos( strtolower( $name ), $operation_get_entity_type )
		)
		{
			...
			)
			{
				...
				if (
					in_array( $method_name, get_class_methods( $class_entity ) )
				)
		...
		if ( ! isset( $callback_parameters ) )
			...
			return $class_entity::__callStatic(
				str_replace( CLASS_ENTITY, '', $name ), $arguments
			);
...
*
* Implement use of Order By clause
*
* method affected ::
*
* 	DATA_FETCHER :: fetchProperties
