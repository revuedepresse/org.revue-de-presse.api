...
See spec §9 (`doc/superpowers/specs/2026-05-17-nestjs-api-port-design.md`). Short version: deploy alongside PHP, ramp reverse-proxy weight 0% → 1% → 5% → 25% → 100%, watch `x-cache` hit-rates and p95 latency match. Roll back at any step by flipping the weight.
