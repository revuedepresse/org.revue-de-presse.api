...
clone-emoji-repository: ## Clone emoji repository
		@/bin/bash -c 'source ./bin/console.sh && clone_emoji_repository'
...
dispatch-messages-from-aggregates-lists: ## Produce messages from aggregates list
		@/bin/bash -c 'source ./bin/console.sh && dispatch_messages_for_aggregates_list'

dispatch-messages-from-member-timeline: ## Produce messages from member timeline
		@/bin/bash -c 'source ./bin/console.sh && dispatch_messages_from_member_timeline'

dispatch-messages-from-members-lists: ## Produce messages from members lists
		@/bin/bash -c 'source ./bin/console.sh && dispatch_messages_from_members_lists'

dispatch-messages-from-networks: ## Produce messages for networks
		@/bin/bash -c 'source ./bin/console.sh && dispatch_messages_for_networks'

dispatch-messages-from-news-list: ## Produce messages from news list
		@/bin/bash -c 'source ./bin/console.sh && dispatch_messages_for_news_list'

dispatch-messages-from-search-query: ## Produce messages from search query
		@/bin/bash -c 'source ./bin/console.sh && dispatch_messages_for_search_query'

dispatch-messages-from-timely-statuses: ## Produce messages for timely statuses
		@/bin/bash -c 'source ./bin/console.sh && dispatch_messages_for_timely_statuses'
...
run-stack: ## Run stack and its dependencies
		@/bin/bash -c 'source ./bin/console.sh && run_development_stack'
...
diff-schema-of-read-database: ## Generate schema migrations scripts
		@/bin/bash -c 'export PROJECT_DIR='/var/www/devobs'; source ./bin/console.sh && compute_schema_differences_for_read_database'

diff-schema-of-write-database: ## Generate schema migrations scripts
		@/bin/bash -c 'export PROJECT_DIR='/var/www/devobs'; source ./bin/console.sh && compute_schema_differences_for_write_database'

migrate-schema-of-read-database: ## Migrate the read database schema
		@/bin/bash -c 'export PROJECT_DIR='/var/www/devobs'; source ./bin/console.sh && migrate_schema_of_read_database'

migrate-schema-of-write-database: ## Migrate the write database schema
		@/bin/bash -c 'export PROJECT_DIR='/var/www/devobs'; source ./bin/console.sh && migrate_schema_of_write_database'
...
stop-stack: ## Stop development stack
		@/bin/bash -c 'source ./bin/console.sh && stop_development_stack'
